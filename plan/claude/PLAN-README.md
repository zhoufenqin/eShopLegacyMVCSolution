...
A reusable methodology for AI-assisted codebase modernization. Use this plan to give Claude Code (or any AI coding agent) a consistent, high-quality approach to migrating legacy projects.

## What this is

`PLAN.md` captures a structured 5-phase approach to legacy modernization:
...
### Step 1: Add the plan to your repo

Copy `PLAN.md` into the root of the project you want to modernize. Or, if you prefer to keep it separate, place it in a shared location and reference it.
...
the methodology in PLAN.md.

When creating a migration plan, use the phase structure and step
ordering from the plan. Do not skip the verification gates.
```

Alternatively, you can paste the plan content directly into `CLAUDE.md` if you want it always loaded.
...
The plan defines two mandatory gates. A migration is not considered complete unless both pass:
...
The plan is a starting point. Adapt it to your team's needs:
...
- **Not a refactoring methodology.** The plan explicitly separates migration from refactoring. Migrate first, refactor later.
